...
% --- hyperref: safe PDF string mapping (place immediately after \usepackage{hyperref}) ---
\hypersetup{%
  pdftitle={Controllability and Efficient Gramian Computation for Periodic Sylvester Matrix Systems},%
  pdfauthor={M. S. V. D. Sudarsan},%
  hidelinks % suppress link colors/boxes
}

\makeatletter
\pdfstringdefDisableCommands{%
  % Ignore frontmatter macros that cannot appear in PDF strings
  \let\cortext\relax
  \let\corref\relax
  \let\cnotenum\relax
  \let\@corref\relax
  ...
  \let\address\relax
  \let\thanks\relax
  % Map a few math macros to plain text for bookmarks
  \def\Ktil{K}%
  \def\Acal{A}%
  \def\vect{vec}%
  \def\ones{ones}%
...
\hspace*{-0.6cm} % <--- adjust this value to move table left/right
\begin{tabular}{@{}ccccc@{}}
\toprule
