...
%
% Quadrature convergence analysis for Example 1 system
%
...
N_values = [11, 21, 31, 41, 51, 61, 71, 81, 91, 101, 121, 141, 161, 181,
201];
...
W_ref = compute_periodic_gramian_block(A_func, B_func, K_func, T,
N_values(end));
...
N, sigma_min_values(i), kappa_values(i), error_val,
computation_times(i));
...
rel_change = abs(sigma_min_values(i) - sigma_min_values(i-1)) /
sigma_min_values(i-1);
...
fprintf('\n! Convergence not yet achieved - need more quadrature
points\n');
...
final_sigma, EXPECTED_SIGMA_MIN, abs(final_sigma -
EXPECTED_SIGMA_MIN)/EXPECTED_SIGMA_MIN);

fprintf(' κ: %.6f (expected: %.6f, error: %.2e)\n', ...

final_kappa, EXPECTED_KAPPA, abs(final_kappa -
...
figure('Name', 'Convergence Analysis', 'Position', [100, 100, 1000,
400]);
...
semilogx(N_values, sigma_min_values, 'bo-', 'LineWidth', 1.5,
'MarkerSize', 6);

hold on;

semilogx([N_values(1), N_values(end)], [sigma_ref, sigma_ref], 'r--',
'LineWidth', 1);

semilogx([N_values(1), N_values(end)], [EXPECTED_SIGMA_MIN,
EXPECTED_SIGMA_MIN], 'g--', 'LineWidth', 1);
...
legend('\sigma_{min}(W)', 'Reference value', 'Expected value',
'Location', 'best');
...
legend('Relative error', sprintf('Converged at N=%d',
N_values(converged_idx)), ...
