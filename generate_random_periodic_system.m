...
%
% Generate random T-periodic system matrices for testing
%
...
%% Status: Under Review, 2026
% Date: 2026
function [A_func, B_func, K_func] = generate_random_periodic_system(n,
m, T, varargin)

%GENERATE_RANDOM_PERIODIC_SYSTEM Generate random T-periodic system
matrices
...
% Creates a random matrix with all eigenvalues having negative real
parts
...
warning('Failed to generate stable matrix, trying alternative
approach');
...
%ROBUSTNESS_TEST Test algorithm robustness under various challenging
conditions
...
fprintf('Testing algorithm robustness under challenging
conditions\n\n');
...
if i > 1 && epsilon_values(i-1) / eps == 100 % Factor of 100 reduction
in ε
...
W_ill = compute_periodic_gramian_block(A_ill_func, B_ill_func,
K_ill_func, T, N);
...
fprintf('%.0e %.3e %.4f s %s\n', actual_cond, sigma_min_ill, comp_time,
status);
...
W_quad = compute_periodic_gramian_block(A_test, B_test, K_test, T,
N_test);
...
fprintf('%2d %.6e %.3e %.4f\n', N_test, sigma_min_quad, rel_change,
quad_time);
...
W_prec = compute_gramian_with_tolerance(A_prec, B_prec, K_prec, T, 41,
rel_tol, abs_tol);
...
is_symmetric = norm(W_prec - W_prec', 'fro') / norm(W_prec, 'fro') <
1e-10;
...
fprintf('%.0e %.0e %.6e %.4f %s\n', rel_tol, abs_tol, sigma_min_prec,
prec_time, status);
...
[A_ext, B_ext, K_ext] = generate_random_periodic_system(n_extreme, 1, T,
'seed', 12345);
...
fprintf('✓ Near-singular systems: Algorithm handles O(ε²) scaling
correctly\n');
...
function W = compute_gramian_with_tolerance(A_func, B_func, K_func, T,
N, rel_tol, abs_tol)

%COMPUTE_GRAMIAN_WITH_TOLERANCE Modified computation with specific ODE
tolerances
